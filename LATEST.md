...
`docker pull ghcr.io/conceptkernel/ck-lib-js:1.5.6` → declare as a `static_web` (routed) or `layer_sources` (additive merge) entry in your `bundle.yaml` per SPEC.OCI.BUNDLE.v0.4. The bundle lands the facade + transport + cache at image root (`/ck.js`, `/ck-client.js`, `/ck-store.js`, `/vendor/`) ready for spec-standard `COPY --from=cklib_source / dest/`.

| arch  | Pull URI                                | Also tagged | Digest                                                                  | Created (UTC)       |
|-------|-----------------------------------------|-------------|-------------------------------------------------------------------------|---------------------|
| amd64 | `ghcr.io/conceptkernel/ck-lib-js:1.5.6` | `latest`  | `sha256:aaf680cc0575b214a84d13b1e3783e3cdec109ed09f9cba25bf9631215dd8e01` | 2026-07-28 16:34:34 UTC |
| arm64 | `ghcr.io/conceptkernel/ck-lib-js:1.5.6` | `latest`  | `sha256:b257e11614ef839dbc0bef1af9cd7c38dde8ad0adb74bdefbf05f17233c200a2` | 2026-07-28 16:34:34 UTC |

|                       |                                                                                                |
|-----------------------|------------------------------------------------------------------------------------------------|
| Artifact type         | OCI image index (multi-arch); `org.opencontainers.image.designation=ckp:static`               |
| Aggregate index       | `ghcr.io/conceptkernel/ck-lib-js:1.5.6` (also tagged `latest`)                              |
| Aggregate digest      | `sha256:c405f90c2edee6000d49b26c450f03e28a6d0dda36d8addf77d486fc21e985b4`                                                                                    |
| Provenance            | SLSA Build Provenance v1, Sigstore-backed, pushed as OCI referrer                              |
| Built by              | [Workflow run #30378825570](https://github.com/ConceptKernel/CK.Lib.Js/actions/runs/30378825570)                          |
| Built from commit     | [`6835d281c436f455b37c85e8a1900cee5380a3b6`](https://github.com/ConceptKernel/CK.Lib.Js/commit/6835d281c436f455b37c85e8a1900cee5380a3b6)                                          |
| Verify (CLI)          | `gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.6 --repo ConceptKernel/CK.Lib.Js`            |
| Release notes         | https://github.com/ConceptKernel/CK.Lib.Js/releases/tag/v1.5.6                                                  |
...
gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.6 \
  --repo ConceptKernel/CK.Lib.Js

# A specific per-arch leaf
gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js@sha256:aaf680cc0575b214a84d13b1e3783e3cdec109ed09f9cba25bf9631215dd8e01 \
  --repo ConceptKernel/CK.Lib.Js
```

A successful verify means: signed by GitHub's Fulcio CA against the OIDC token of the v1.5.6 `oci-publish` workflow run, recorded in Sigstore's Rekor transparency log, subject digest matches the pulled artifact.
...
static_web:
  - source_image: ghcr.io/conceptkernel/ck-lib-js:1.5.6
    route: /cklib
...
The build MUST run `gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.6 --repo ConceptKernel/CK.Lib.Js` before the consuming image is pushed (SPEC.OCI.BUNDLE.v0.3 §4 build-time gate).
...
- Tagged versions are immutable on GHCR. Pin by version (`1.5.6`) in production bundles; use `latest` only for development.
...
> Rendered automatically by `.github/workflows/oci-publish.yml` on 2026-07-28 16:34:34 UTC after `gh attestation verify` accepted the aggregate digest above.
