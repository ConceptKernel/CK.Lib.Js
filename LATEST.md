...
`docker pull ghcr.io/conceptkernel/ck-lib-js:1.5.10` → declare as a `static_web` (routed) or `layer_sources` (additive merge) entry in your `bundle.yaml` per SPEC.OCI.BUNDLE.v0.4. The bundle lands the facade + transport + cache at image root (`/ck.js`, `/ck-client.js`, `/ck-store.js`, `/vendor/`) ready for spec-standard `COPY --from=cklib_source / dest/`.

| arch  | Pull URI                                | Also tagged | Digest                                                                  | Created (UTC)       |
|-------|-----------------------------------------|-------------|-------------------------------------------------------------------------|---------------------|
| amd64 | `ghcr.io/conceptkernel/ck-lib-js:1.5.10` | `latest`  | `sha256:14c0db1287bca7f473a03650c6e213bfae20223f1a9af2a7dcb1f2c520face14` | 2026-08-11 09:07:16 UTC |
| arm64 | `ghcr.io/conceptkernel/ck-lib-js:1.5.10` | `latest`  | `sha256:12c557d5ae4c0c9951b3df729ba25425e97505ad24f4172ebb58449f9879d7db` | 2026-08-11 09:07:16 UTC |

|                       |                                                                                                |
|-----------------------|------------------------------------------------------------------------------------------------|
| Artifact type         | OCI image index (multi-arch); `org.opencontainers.image.designation=ckp:static`               |
| Aggregate index       | `ghcr.io/conceptkernel/ck-lib-js:1.5.10` (also tagged `latest`)                              |
| Aggregate digest      | `sha256:e56ce6515ad7524d1b9570280c36ae5c2e1140cf640496151de7e684eca95e53`                                                                                    |
| Provenance            | SLSA Build Provenance v1, Sigstore-backed, pushed as OCI referrer                              |
| Built by              | [Workflow run #31476188141](https://github.com/ConceptKernel/CK.Lib.Js/actions/runs/31476188141)                          |
| Built from commit     | [`21b3e41cb29d12ca1cabfd08c4fa0016b26dd2be`](https://github.com/ConceptKernel/CK.Lib.Js/commit/21b3e41cb29d12ca1cabfd08c4fa0016b26dd2be)                                          |
| Verify (CLI)          | `gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.10 --repo ConceptKernel/CK.Lib.Js`            |
| Release notes         | https://github.com/ConceptKernel/CK.Lib.Js/releases/tag/v1.5.10                                                  |
...
gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.10 \
  --repo ConceptKernel/CK.Lib.Js

# A specific per-arch leaf
gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js@sha256:14c0db1287bca7f473a03650c6e213bfae20223f1a9af2a7dcb1f2c520face14 \
  --repo ConceptKernel/CK.Lib.Js
```

A successful verify means: signed by GitHub's Fulcio CA against the OIDC token of the v1.5.10 `oci-publish` workflow run, recorded in Sigstore's Rekor transparency log, subject digest matches the pulled artifact.
...
static_web:
  - source_image: ghcr.io/conceptkernel/ck-lib-js:1.5.10
    route: /cklib
...
The build MUST run `gh attestation verify oci://ghcr.io/conceptkernel/ck-lib-js:1.5.10 --repo ConceptKernel/CK.Lib.Js` before the consuming image is pushed (SPEC.OCI.BUNDLE.v0.3 §4 build-time gate).
...
- Tagged versions are immutable on GHCR. Pin by version (`1.5.10`) in production bundles; use `latest` only for development.
...
> Rendered automatically by `.github/workflows/oci-publish.yml` on 2026-08-11 09:07:16 UTC after `gh attestation verify` accepted the aggregate digest above.
